 ...
 *
 * Uso:
 *   npm run sync-cotacoes-us
 *   npm run sync-cotacoes-us -- --job-id 42   (com rastreamento de progresso via status_cron_job)
 */
import './env';
...
async function main(): Promise<CotacaoSyncResult> {
	...
	const { data: ativos, error: listError } = await supabase.from('ativos').select('code').in('type', ['stock', 'reit']);
	...
	console.log(`Sincronizando ${rows.length} ativo(s) US em ${batches.length} lote(s)...`);
